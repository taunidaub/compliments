...
*/
	...
	function LDAPConnect($ADUsername,$ADPassword){
		...
		$ADhost			= 'ALBCORPDC11.corp.domain.com';
		$ADhost2		= 'ALBCORPDC12.corp.domain.com';
		...
		if ($ds) {																		// If connection is good then attempt to bind to server
			$ldapbind = ldap_bind ( $ds, 'CompanyCORP\\'.$ADUsername, $ADPassword);			// Binding to ldap server
...
if ( ! function_exists('getOthersFName'))
{
/*
Function: getOthersFName
	...
	Example:
		(code)
			$Full_Name = getOthersFName($EID,$Password,$OEID);
		(end)

*/
    function getOthersFName($EID,$Password,$OEID){														
		$ds = LDAPConnect($EID,$Password);
		$Perms = 3;
		$dn = 'OU=Company Divisions,DC=corp,DC=domain,DC=com';								// Set the base DN
		$filter='(&(objectCategory=*)(cn='.$OEID.'*))';									// Create the filter (more Detal = faster)
...
if ( ! function_exists('getOthersLName'))
{
/*
Function: getOthersLName
	...
	Example:
		(code)
			$Full_Name = getOthersLName($EID,$Password,$OEID);
		(end)

*/
    function getOthersLName($EID,$Password,$OEID){														
		$ds = LDAPConnect($EID,$Password);
		$Perms = 3;
		$dn = 'OU=Company Divisions,DC=corp,DC=domain,DC=com';								// Set the base DN
		$filter='(&(objectCategory=*)(cn='.$OEID.'*))';									// Create the filter (more Detal = faster)
...
*/
    function LDAP_authUser($EID,$Password){
		...
		if ($ds) {																		// If connection is good then attempt to bind to server
			if (ldap_bind ( $ds, 'CompanyCORP\\'.$EID, $Password)) {						// If We bind ok then return 1.
				return 1 ;
			}else{
